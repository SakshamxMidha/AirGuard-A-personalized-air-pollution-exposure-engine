...
cp .env.example .env
# Edit .env: set JWT\\\\\\\_SECRET and DB\\\\\\\_PASSWORD
...
```bash
JWT\\\\\\\_SECRET=your-random-256bit-secret
DATABASE\\\\\\\_URL=jdbc:postgresql://host:5432/airguard
DB\\\\\\\_USER=airguard
DB\\\\\\\_PASSWORD=secure-password
ALLOWED\\\\\\\_ORIGINS=https://yourdomain.com
```
...
* \[ ] Generate `JWT\\\\\\\_SECRET`: `openssl rand -hex 32`
* \[ ] Set `SPRING\\\\\\\_PROFILES\\\\\\\_ACTIVE=prod`
* \[ ] Configure PostgreSQL `DATABASE\\\\\\\_URL`, `DB\\\\\\\_USER`, `DB\\\\\\\_PASSWORD`
* \[ ] Set `ALLOWED\\\\\\\_ORIGINS` to your actual domain
